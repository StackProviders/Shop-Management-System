...
function useDataGrid<TData>({
            ...
            for (const update of updateArray) {
                if (!rows || !currentTable) {
                    const existingUpdates =
                        rowUpdatesMap.get(update.rowIndex) ?? []
                    ...
                    const existingUpdates =
                        rowUpdatesMap.get(originalRowIndex) ?? []
                    existingUpdates.push({
                        columnId: update.columnId,
                        value: update.value
                    })
    ...
    const getIsCellSelected = React.useCallback(
        (rowIndex: number, columnId: string) => {
            return selectionState.selectedCells.has(
        ...
        (rowIndex: number, columnId: string, event?: React.MouseEvent) => {
            ...
            if (event) {
                ...
                if (event.shiftKey && currentState.focusedCell) {
                    event.preventDefault()
                    selectRange(currentState.focusedCell, {
                        rowIndex,
                        columnId
                    })
                    return
        ...
        (event: KeyboardEvent) => {
            ...
            if (key === 'Delete' || key === 'Backspace') {
                if (currentState.selectionState.selectedCells.size > 0) {
                    ...
                    currentState.selectionState.selectedCells.forEach(
                        (cellKey) => {
        ...
        function onGlobalKeyDown(event: KeyboardEvent) {
            ...
            if (enableSearch && isCtrlPressed && key === SEARCH_SHORTCUT_KEY) {
                ...
                const isInSearchInput =
                    target.closest('[role="search"]') !== null
