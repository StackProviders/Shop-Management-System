        ...
        <DialogPortal>
            ...
            <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
                <DialogPrimitive.Popup
                    ...
                    className={cn(
                        'grid w-full min-w-0 gap-4 border bg-popover bg-clip-padding p-6 text-popover-foreground shadow-lg transition-[scale,opacity,translate] duration-200 ease-in-out will-change-transform data-ending-style:opacity-0 data-starting-style:opacity-0 max-sm:max-h-[85vh] max-sm:overflow-y-auto max-sm:border-none sm:max-w-lg sm:rounded-2xl sm:data-ending-style:scale-98 sm:data-starting-style:scale-98 dark:bg-clip-border',
...
function DialogFooter({ className, ...props }: React.ComponentProps<'div'>) {
    return (
        <div
            ...
            className={cn(
                'flex flex-col-reverse gap-2 sm:flex-row sm:justify-end sm:border-t sm:bg-muted/50 sm:px-6 sm:py-4 sm:rounded-b-xl sm:-mx-6 sm:-mb-6 sm:mt-4',
                className
            )}
            {...props}
        />
